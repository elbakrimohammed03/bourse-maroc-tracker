...
import time
import os
import numpy as np  # Indispensable pour nettoyer les données
...
def clean_val(val):
    """Nettoie et valide les nombres pour éviter les erreurs JSON."""
    try:
        if val is None or str(val).strip() in ['', '-', 'N/A']: return None
        res = re.sub(r'[^\d.-]', '', str(val).replace(',', '.'))
        f_val = float(res)
        # Vérification anti-NaN et anti-Infini
...
# --- ÉTAPE 3 : FUSION ET NETTOYAGE JSON FINAL ---
...
if not df_base.empty and not df_details.empty:
    ...
    # Nettoyage global du DataFrame pour les valeurs hors limites
    ...
    print("⚠️ Échec de la fusion des données.")
