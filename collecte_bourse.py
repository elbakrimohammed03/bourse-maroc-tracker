...
for a in soup.find_all('a', href=True):
    if '/action/capitalisation' in a['href']:
        ...
        name = a.get_text(strip=True).upper() # Force Majuscule pour le merge
...
for row in soup.find_all('tr'):
    ...
    if len(cols) == 9:
        rows_base.append({
            'entreprise': cols[1].get_text(strip=True).upper(), # Force Majuscule
...
# --- ÉTAPE 3 : FUSION ET ENVOI ---
df_base = pd.DataFrame(rows_base)
df_details = pd.DataFrame(all_details)

if not df_base.empty and not df_details.empty:
    df_final = df_base.merge(df_details, on='entreprise', how='inner') # Inner pour éviter les lignes vides
    df_final['date_collecte'] = datetime.now().strftime("%Y-%m-%d")
    
    # Conversion finale pour Supabase (NaN -> None)
    ...
    try:
        ...
        print("✅ Données insérées avec succès dans bourse_details !")
    except Exception as e:
        print(f"❌ Erreur lors de l'insertion : {e}")
else:
    print("⚠️ Aucune donnée n'a pu être fusionnée.")
